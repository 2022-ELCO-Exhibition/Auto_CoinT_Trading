...
from lab.trader.environment import Environment
from lab.trader.agent import Agent
from lab.trader.networks import Network, DNN, LSTMNetwork, CNN
from lab.trader.visualizer import Visualizer
from lab.trader import utils
from lab.trader import settings

logger = logging.getLogger(settings.LOGGER_NAME)

print(os.path.isfile("test.html"))
...
class ReinforcementLearner:
    ...
    def run(self, learning=True):
        ...
        for epoch in tqdm(range(self.num_epoches)):
            ...
            print(f'[{self.stock_code}][Epoch {epoch_str}/{self.num_epoches}] '
                  f'Epsilon:{epsilon:.4f} #Expl.:{self.exploration_cnt}/{self.itr_cnt} '
                  f'#Buy:{self.agent.num_buy} #Sell:{self.agent.num_sell} #Hold:{self.agent.num_hold} '
                  f'#Stocks:{self.agent.num_stocks} PV:{self.agent.portfolio_value:,.0f} '
                  f'Loss:{self.loss:.6f} ET:{elapsed_time_epoch:.4f}')
